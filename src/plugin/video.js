      ...
      if (isUrl) {
        ...
        videoStream.on('end', async () => {
          try {
            ...
            await Matrix.sendMessage(m.from, { video: finalVideoBuffer, mimetype: 'video/mp4', caption: '> © Powered by Queen_Anita-Md' }, { quoted: m });
            await m.React("✅");
          } catch (err) {
        ...
        videoStream.on('end', async () => {
          try {
            ...
            await Matrix.sendMessage(m.from, { video: finalVideoBuffer, mimetype: 'video/mp4', caption: '> © Powered by 𝞢𝙏𝞖𝞘𝞦-𝞛𝘿' }, { quoted: m });
            await m.React("✅");
          } catch (err) {
            console.error('Error sending video:', err);
            m.reply('Error sending video.');
